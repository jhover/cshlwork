...
def download_run_sra(runid, outdir=None):
    srcurl=f'{URLBASE}/{runid}/{runid}'
    try:
        if outdir is not None:
            outdir = os.path.abspath(outdir)
            download_wget(srcurl, destpath=f'{outdir}/{runid}.sra', rate = '50M')
        else:
...
def get_default_config():
    cp = ConfigParser()
    cp.read(os.path.expanduser("~/git/cshlwork/etc/sra.conf"))
    return cp
...
class FasterqDump(object):
    ...
    def __init__(self, config, outdir, run_id): #, outlist
        self.log = logging.getLogger('sra')
        self.run_id = run_id
        self.outdir = outdir
